...
// Author: K. Sung
// Purpose:
//     Go to the bottom of the file and examine the global function definitions
//
#include "Point.h"
#include <iostream>
using namespace std;
...
// --------------------------------------------------------------
// Global function definitions for Point class

// Note: these functions are NOT members of the Point class 
//       (the absence of Point:: before the function name)
//       but they have access to private data members of Point class
void f(const Point &p) {
    cout << "Global f(): " << p.x << ", " << p.y << endl; // these are private data members!
}

Point& operator--(Point &p) {  // this is pre-decrement (int dummy int)
    p.x--; p.y--; // these are private data members!
    return p;
}

Point operator-(const Point &p1, int scaler) { // binary minus operator
    return Point("global-op-", p1.x - scaler, p1.y - scaler); // these are private data members!
}
